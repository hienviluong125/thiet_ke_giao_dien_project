  ...
  return (
    <View style={{ backgroundColor: "#ffffff", height: "100%" }}>
      <Portal style={{ padding: 0, margin: 0 }}>
        ...
        >
          <MaterialCommunityIcons
            ...
            style={{ color: "#4CAF50", fontSize: 50, textAlign: "center" }}
          />
          <Text style={{ fontSize: 18, textAlign: "center", marginTop: 10 }}>
            Bạn đã hoàn thành bài trắc nghiệm với số câu đúng là 14/16.
          </Text>
          <Button
            style={{ backgroundColor: "#4CAF50", marginTop: 15 }}
            color="white"
            labelStyle={{ fontSize: 17 }}
            onPress={hideModal}
            onPress={() => {
              navigation.navigate("QuizResult", { quizLession: "Nguyên tử" });
              setVisible(false);
            }}
          >
            Xem kết quả
      ...
      <View style={styles.breadcum}>
        <Text style={styles.breadcumText}>Bài học</Text>

        <Icon name="chevron-right" style={styles.chevronIcon} />

        <Text style={styles.breadcumText}>Hóa học 10</Text>
      ...
      <ScrollView style={styles.items}>
        {lessions.map((lession, index) => (
          <Card
            style={[
              styles.cardStyle,
              index === lessions.length - 1 ? { marginBottom: 20 } : {},
            ]}
            onPress={() =>
              navigation.navigate("Quiz", { quizLession: lession.name })
            }
          >
            <Card.Content>
                ...
                <View style={styles.progressBarWrapper}>
                  <View
                    style={[
                      ...
                      ,
                      { width: `${(lession.done / lession.total) * 100}%` },
                    ]}
                  ></View>
                  <View
                    style={[
                      styles.nonActiveProgressBar,
                      { width: `${(1 - lession.done / lession.total) * 100}%` },
                    ]}
                  ></View>
                </View>
                <View>
                  <Text style={{ fontWeight: "400" }}>
                    {lession.done}/{lession.total}
                  </Text>
                </View>
              </View>
            </Card.Content>
